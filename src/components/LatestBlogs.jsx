...
const LatestBlogs = () => {
  ...
  const LOCAL_KEY = "latest_blogs_cache_toyvista";
            ...
            >
              ...
              <Link to={`/blog/${post.slug}`} state={{ blog: post }} className="relative block overflow-hidden rounded-t-2xl">
                <img
                  ...
                  loading="lazy"
                />
                <span className="absolute px-3 py-1 text-xs font-semibold text-white rounded-full top-3 left-3 bg-gradient-to-r from-blue-600 to-green-500">
                  Blog
                </span>
              </Link>
              ...
              <div className="flex flex-col flex-grow p-5">
                <Link to={`/blog/${post.slug}`} state={{ blog: post }}>
                  <h3 className="text-lg font-semibold text-gray-900 font-[Fredoka] line-clamp-2 hover:text-blue-600 transition-colors duration-200">
                    {post.title}
                  </h3>
                </Link>
                ...
                <div className="flex items-center justify-between mt-4">
                  ...
                  <Link
                    to={`/blog/${post.slug}`}
                    state={{ blog: post }}
                    className="px-4 py-1 text-xs font-semibold text-white transition-opacity duration-200 rounded-full shadow bg-gradient-to-r from-blue-600 to-green-500 hover:opacity-90"
                  >
                    Read More
                  </Link>
                </div>
