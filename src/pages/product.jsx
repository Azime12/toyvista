...
export default function ProductList() {
  ...
  <button
    onClick={() => navigate(-1)}
    className="flex items-center mb-4 text-sm text-[#007BFF] hover:text-[#50A8FF] transition-colors"
  >
    <svg 
    ...
    {paginatedProducts.map((product) => (
      ...
      >
        ...
        <div className="p-4">
          ...
          <button
            onClick={() => goToDetail(product)}
            className="w-full px-4 py-3 mt-4 font-semibold text-white transition-all duration-300 bg-gradient-to-r from-[#66CB67] to-[#007BFF]/90 rounded-lg hover:from-[#74d676]/90 hover:to-[#007BFF] hover:shadow-lg active:scale-95"
          >
            View Details
          </button>
        </div>
  ...
  {products.length > PAGE_SIZE && (
    <div className="flex flex-col items-center justify-center mt-10 space-y-4 md:flex-row md:space-y-0 md:space-x-4">
      <button
        ...
        className={`flex items-center justify-center px-5 py-3 border rounded-xl transition-all duration-300 ${
          currentPage === 1
            ...
            : "bg-white text-[#007BFF] border-[#007BFF]/30 hover:bg-[#007BFF] hover:text-white hover:border-[#007BFF] hover:shadow-md"
        }`}
      >
        <svg 
          xmlns="http://www.w3.org/2000/svg" 
      ...
      <div className="flex items-center space-x-2">
        ...
        <span className="px-4 py-2 font-bold text-white bg-[#50A8FF] rounded-lg min-w-[60px] text-center">
          {currentPage}
        </span>
        <span className="px-4 py-2 font-medium text-gray-600">
          of {totalPages}
